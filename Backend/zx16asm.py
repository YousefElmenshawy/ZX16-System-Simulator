...
class ZX16Parser:
    ...
    def expand_pseudo_instruction(self, mnemonic: str, operands: List[Union[int, str]], current_pc: int = 0,
                                  symbol_resolver=None) -> List[Tuple[str, List[Union[int, str]]]]:
                    ...
                    if mnemonic == 'li':
                        if len(operands) >= 2:
                            ...
                            if -64 <= imm <= 63:
                                ...
                                expanded = parser.expand_pseudo_instruction('li16', [rd, imm], self.current_address,
                                                                            symbol_resolver)
                        ...
                        expanded = parser.expand_pseudo_instruction(mnemonic, operands, self.current_address,
                                                                    symbol_resolver)
                        for exp_mnemonic, exp_operands in expanded:
                            encoding = self.encode_instruction(exp_mnemonic, exp_operands, parser)
                            if isinstance(encoding, int):
