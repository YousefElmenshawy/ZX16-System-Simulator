    ...
    def tokenize(self) -> List[Token]:
        ...
        while self.pos < len(self.text):
            ...
            if self.current_char().isalpha() or self.current_char() == '_':
                ...
                # Check if identifier is immediately followed by a colon to mark a label
                if self.current_char() == ':':
                    self.tokens.append(Token(TokenType.LABEL, identifier, line, column))
                    logging.debug(f"Label token added: {identifier}")
...
class ZX16Assembler:
    ...
    def encode_instruction(self, mnemonic: str, operands: list, parser: 'ZX16Parser') -> int:
        ...
        # B-Type instructions
            ...
            # Resolve symbolic branch target if necessary
            if isinstance(target, str):
                target = self.resolve_symbol(target, parser.current_token.line)
        ...
        # J-Type instructions
            ...
            # Resolve symbol for jump target if necessary
            if isinstance(target, str):
                target = self.resolve_symbol(target, parser.current_token.line)
